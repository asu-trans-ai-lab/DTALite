...
void write_default_demand_period_file_if_not_exist()
{
    ...
    if (inFile.good())
    {
        dtalog.output() << "The file " << filename << " already exists.\n";
        return;
    }
...
void write_default_departure_time_profile_if_not_exist()
{
    ...
    ofstream outFile(filename);
    // Include time_period in the header
    outFile << "first_column,departure_time_profile_no,time_period,";
    ...
    for (const auto& item : data)
    {
        // Include time_period in the data writing
...
void write_default_subarea_file_if_not_exist()
{
    ...
    if (!inFile.good())
    {
        ...
        dtalog.output() << "The file " << filename << " was generated successfully!\n";
    }
    else
    {
        dtalog.output() << "The file " << filename << " already exists.\n";
...
void write_default_dynamic_traffic_management_file_if_not_exist()
{
    ...
    outFile.close();
    dtalog.output() << "The file " << filename << " has been created with default values.\n";
}
...
int main()
{
    ...
    dtalog.output() << "Logbook for DTALite: The Open-Source, Lightweight Dynamic Traffic Assignment Solution"
        			<< fixed << setw(12) << '\n';
    dtalog.output() << " Overview of files and process\n";
    dtalog.output() << "1. Input Files:\n";
    dtalog.output() << "   |--- Physical Layer (node.csv, link.csv)\n";
    dtalog.output() << "   |--- Demand Layer (demand.csv, mode_type.csv, demand_period.csv, "
	...
	dtalog.output() << "Please provide feedback or report any issues you encounter on our GitHub site: "
        			<< "https://github.com/asu-trans-ai-lab/DTALite/issues. Your input helps us enhance the "
           			<< "software, address any concerns, and contribute to the open-source transportation ecosystem.\n";

    g_DTA_log_file << "Logbook for DTALite: The Open-Source, Lightweight Dynamic Traffic Assignment Solution"
                   << fixed << setw(12) << '\n';
    g_DTA_log_file << " Overview of files and process\n";
    g_DTA_log_file << "1. Input Files:\n";
    g_DTA_log_file << "   |--- Physical Layer (node.csv, link.csv)\n";
    g_DTA_log_file << "   |--- Demand Layer (demand.csv, mode_type.csv, demand_period.csv, "
                   << "departure_time_profile.csv, demand_file_list.csv, sensor_data.csv, subarea.csv)\n";
    ...
    g_DTA_log_file << "    dynamic_traffic_management.csv: Defines different dynamic traffic management scenarios.\n";
    ...
    g_DTA_log_file << "  route_assignment_s(scenario_index)_(scenario_name).csv: Shows the results of the "
    ...
    g_DTA_log_file << "  system_performance_summary.csv: Shows the performance of the whole transportation "
    ...
    {
        if (parser_settings.CheckingSettingFormat())
        {
    ...
    if (bCorrectSettingFormat)
    {
        ...
        if (parser_settings.GetValueByKeyName("number_of_iterations", number_of_iterations, false))
        ...
        if (CheckSupplySideScenarioFileExist())
        {
        ...
        if (parser_settings.GetValueByKeyName("UE_convergence_percentage",
                                              UE_convergence_percentage, false, false))
        {
        ...
        if (parser_settings.GetValueByKeyName("number_of_column_updating_iterations",
                                              column_updating_iterations, false, false))
        {
