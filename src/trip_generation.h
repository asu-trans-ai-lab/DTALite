...
void g_add_new_access_link(int internal_from_node_seq_no,
                           int internal_to_node_seq_no,
                           float link_distance_VDF,
                           int mode_type_no,
                           int zone_seq_no = -1)
{
    ...
    for (int tau = 0; tau < assignment.g_number_of_demand_periods; ++tau)
    {
        ...
        link.free_flow_travel_time_in_min =
            link_distance_VDF / std::max(0.001, link.free_speed) * 60;
    ...
    g_node_vector[internal_from_node_seq_no].m_outgoing_link_seq_no_vector.push_back(
        link.link_seq_no);
    // add this link to the corresponding node as part of outgoing node/link
    g_node_vector[internal_to_node_seq_no].m_incoming_link_seq_no_vector.push_back(
        link.link_seq_no);
    // add this link to the corresponding node as part of outgoing node/link
    g_node_vector[internal_from_node_seq_no].m_to_node_seq_no_vector.push_back(link.to_node_seq_no);
    // add this link to the corresponding node as part of outgoing node/link
    g_node_vector[internal_from_node_seq_no].m_to_node_2_link_seq_no_map[link.to_node_seq_no] =
        link.link_seq_no;
...
double get_activity_node_distance(Assignment& assignment)
{
    ...
    for (int i = 0; i < g_node_vector.size(); i++)
    {
        ...
        if (g_node_vector[i].is_activity_node)
        {
            ...
            for (int j = 0; j < g_node_vector.size(); j++)
            {
                ...
                {
                    double near_by_distance =
                        g_calculate_p2p_distance_in_meter_from_latitude_longitude(
                            g_node_vector[i].x, g_node_vector[i].y, g_node_vector[j].x,
                            g_node_vector[j].y);
    ...
    double nearby_distance = total_near_by_distance / std::max(1, activity_node_count);
...
void g_grid_zone_generation(Assignment& assignment)
{
    ...
    if (parser.OpenCSVFile("node.csv", true))
    {
        ...
        while (parser.ReadRecord())
        {
            ...
            // random generation of activity locations // no boundary nodes
            if (number_of_is_boundary <= 1)
            {
    ...
    fprintf(g_pFileZone,
            "first_column,zone_id,access_node_vector,cell_code,cell_id,access_distance,x_coord,y_"
            "coord,");
    ...
    {
        // expand the grid boundary according to the nodes
        left = std::min(left, l_node_vector[i].x);
        right = std::max(right, l_node_vector[i].x);
        top = std::max(top, l_node_vector[i].y);
        bottom = std::min(bottom, l_node_vector[i].y);
    }
    ...
    {
        if ((temp_resolution > ResolutionVector[i] + 0.000001) &&
            temp_resolution < ResolutionVector[i + 1])
        {
    ...
    for (int i = 0; i < l_node_vector.size(); i++)
    {
        ...
        {
            __int64 cell_id =
                g_get_cell_ID(l_node_vector[i].x, l_node_vector[i].y, assignment.m_GridResolution);
            // create a cell
            if (local_cell_id_2_zone_mapping.find(cell_id) == local_cell_id_2_zone_mapping.end())
            {
                // create zone
    ...
    float production_rate_per_activity_node =
        total_number_of_trips_expected_lower_bound / std::max(1, number_of_activity_nodes);
    ...
    for (int i = 0; i < l_node_vector.size(); i++)
    {
        if (l_node_vector[i].is_activity_node != 0)
        {
            __int64 cell_id =
                g_get_cell_ID(l_node_vector[i].x, l_node_vector[i].y, assignment.m_GridResolution);
            // create a cell
            if (assignment.cell_id_mapping.find(cell_id) == assignment.cell_id_mapping.end())
            {
                // create zone
                assignment.cell_id_mapping[cell_id] = l_node_vector[i].node_id;
                std::string cell_code = g_get_cell_code(l_node_vector[i].x, l_node_vector[i].y,
                                                        assignment.m_GridResolution, left, top);
                ...
                for (int j = 0; j < l_node_vector.size(); j++)
                {
                    ...
                    {
                        __int64 cell_id_j = g_get_cell_ID(l_node_vector[j].x, l_node_vector[j].y,
                                                          assignment.m_GridResolution);
                        if (cell_id == cell_id_j)
                        {
                            double distance =
                                g_calculate_p2p_distance_in_meter_from_latitude_longitude(
                                    zone_x, zone_y, l_node_vector[j].x, l_node_vector[j].y);
                ...
                fprintf(g_pFileZone, ",%s,%d,", cell_code.c_str(),
                        assignment.cell_id_mapping[cell_id]);
                fprintf(g_pFileZone, "%f,", max_distance);

                fprintf(g_pFileZone, "%f,%f,",
                        (l_node_vector[i].x * 2 + x_coord_left + x_coord_right) / 4,
                        (l_node_vector[i].y * 2 + y_coord_top + y_coord_bottom) / 4);
                ...
                for (int at = 0; at < assignment.g_ModeTypeVector.size(); ++at)
                {
                    fprintf(g_pFileZone, "%.2f,%.2f,",
                            access_node_vector.size() * production_rate_per_activity_node,
                            access_node_vector.size() * production_rate_per_activity_node);
                    break;
                }
    ...
    dtalog.output() << "[PROCESS INFO] Step 1.4.3: creating " << assignment.cell_id_mapping.size()
                    << " zones.\n";
    g_DTA_log_file << "[PROCESS INFO] Step 1.4.3: creating " << assignment.cell_id_mapping.size()
                   << " zones.\n";
    fclose(g_pFileZone);
}
...
bool g_TAZ_2_GMNS_zone_generation(Assignment& assignment)
{
    ...
    {
        // fprintf(g_pFileOutputLog, "number of nodes =,%d\n", assignment.g_number_of_nodes);
        while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field
                                     // headers.
        {
            int zone_id;
    ...
    else
    {
        assignment.summary_file
            << "[NOTE] If zone.csv has not been prepared, please prepare TAZ.csv from the planning "
               "model so that DTALite can help with generating access nodes that connect zone to "
               "nodes in the network.\n";
        
		// dtalog.output() << "If zone.csv has not been prepared, please prepare TAZ.csv from the
        // planning model so that DTALite can help with generating access nodes that connect zone to
        // nodes in the network." << '\n'; g_DTA_log_file << "If zone.csv has not been prepared,
        // please prepare TAZ.csv from the planning model so that DTALite can help with generating
        // access nodes that connect zone to nodes in the network." << '\n';
    ...
    // l as local
    std::vector<CNode> l_node_vector;
    if (parser.OpenCSVFile("node.csv", true))
    {
        // if this line contains [] mark, then we will also read field headers.
        while (parser.ReadRecord())
    ...
    dtalog.output() << "[PROCESS INFO] Step 1.4.0: QEM mode for creating TAZ 2 zone mapping with "
                    << l_TAZ_vector.size() << " TAZ and " << l_node_vector.size() << " nodes.\n";
    g_DTA_log_file << "[PROCESS INFO] Step 1.4.0: QEM mode for creating TAZ 2 zone mapping with "
                   << l_TAZ_vector.size() << " TAZ and " << l_node_vector.size() << " nodes.\n";
    ...
    fprintf(g_pFileZone,
            "first_column,zone_id,access_node_vector,access_distance_vector,x_coord,y_coord,access_"
            "link_geometry,\n");
    ...
    for (int taz = 0; taz < l_TAZ_vector.size(); taz++)
    {
        ...
        for (int i = 0; i < l_node_vector.size(); i++)
        {
            ...
            {
                // test                                double near_by_distance_1 =
                // g_calculate_p2p_distance_in_meter_from_latitude_longitude(-77.429293, 39.697895,
                // -77.339847, 38.947676);

                double distance = g_calculate_p2p_distance_in_meter_from_latitude_longitude(
                    zone_x, zone_y, l_node_vector[i].x, l_node_vector[i].y);
        ...
        if (access_node_distance_vector.size() > 0)
        {
            ...
            int acecss_link_k = 4;
            double distance_k_cut_off_value =
                access_node_distance_vector[std::max(0, acecss_link_k - 1)];
        ...
        for (int n = 0; n < access_node_seq_vector.size(); n++)
        {
            ...
            fprintf(g_pFileZone, "%f %f,", zone_x, zone_y);
            fprintf(g_pFileZone, "%f %f,", l_node_vector[node_seq_no].x,
                    l_node_vector[node_seq_no].y);
            fprintf(g_pFileZone, ") ,");
        }
        fprintf(g_pFileZone, ")\"");
    ...
    assignment.summary_file << "[DATA INFO] # of zones created based on zone definition in TAZ.csv=,"
        					<< l_TAZ_vector.size() << '\n';
    assignment.summary_file << "[DATA INFO] # of access nodes created based on node.csv and zone "
                            << "definition in TAZ.csv=," << access_node_size << '\n';
...
void g_create_zone_vector(Assignment& assignment)
{
    ...
    for (std::map<int, int>::iterator it = assignment.zone_id_2_node_no_mapping.begin();
         it != assignment.zone_id_2_node_no_mapping.end(); ++it)
    {
        COZone ozone;
        ...
        dtalog.output() << "[DATA INFO] create zone id = " << ozone.zone_id
                        << " with representive node id " << it->second
                        << ",x = " << g_node_vector[it->second].x << ",y=" << ozone.cell_y << '\n';

        g_DTA_log_file << "[DATA INFO] create zone id = " << ozone.zone_id
                       << " with representive node id " << it->second
                       << ",x = " << g_node_vector[it->second].x << ",y=" << ozone.cell_y << '\n';

        assignment.g_zoneid_to_zone_seq_no_mapping[ozone.zone_id] =
            ozone.zone_seq_no;  // create the zone id to zone seq no mapping
        ...
        for (int dest = 0; dest < g_zone_vector.size(); ++dest)  // d
        {
            ...
            if (orig != dest)
            {
                ...
                float distance_in_mile =
                    g_calculate_p2p_distance_in_meter_from_latitude_longitude(
                        g_zone_vector[orig].cell_x, g_zone_vector[orig].cell_y,
                        g_zone_vector[dest].cell_x, g_zone_vector[dest].cell_y) /
                    1609.0;
                ...
                // Store the calculated travel time in the corresponding cell of the travel time
                // matrix
        ...
        for (int orig = 0; orig < g_zone_vector.size(); ++orig)  // o
        {
            for (int dest = 0; dest < g_zone_vector.size(); ++dest)  // d
            {
                if (g_zone_vector[orig].gravity_production >= 0)
                {
                    ...
                    {
                        float value = -1;
                        if (g_zone_vector[orig].m_ODAccessibilityMatrix.distance_map.find(dest) !=
                            g_zone_vector[orig].m_ODAccessibilityMatrix.distance_map.end())
                        {
                        ...
                        if (value > 0.000001)
                        {
                            fprintf(g_pFileODMatrix, "%d,%d,%.4f,", g_zone_vector[orig].zone_id,
                                    g_zone_vector[dest].zone_id, value);

                            fprintf(g_pFileODMatrix, "\"LINESTRING (");

                            fprintf(g_pFileODMatrix, "%f %f,", g_zone_vector[orig].cell_x,
                                    g_zone_vector[orig].cell_y);
                            fprintf(g_pFileODMatrix, "%f %f,", g_zone_vector[dest].cell_x,
        ...
        if (g_zone_vector[orig].gravity_production > 0.00001)
        {
            ...
            for (int d = 0; d < g_zone_vector.size(); ++d)
            {
                if (orig != d)
                {
                    ...
                    //                            float cut_off =
                    //                            assignment.g_ModeTypeVector[at].trip_time_budget_in_min;
                    float cut_off = 40;
                    if (g_zone_vector[d].gravity_attraction > 0)
                    {
                        // double disutility =
                        // g_zone_vector[orig].m_ODAccessibilityMatrix.value_map[d] * beta;
                        double exp_disutility = g_zone_vector[d].gravity_attraction;
                        g_zone_vector[orig].m_ODAccessibilityMatrix.disutility_map[d] =
                            exp_disutility;
                        ...
                        if (g_zone_vector[orig].m_ODAccessibilityMatrix.value_map[d] < cut_off)
                        {
                            ...
                            {
                                dtalog.output()
                                    << ", o: " << orig << ",d:" << d << ", gc distance = "
                                    << g_zone_vector[orig].m_ODAccessibilityMatrix.distance_map[d]
                                    << ", travel time ="
                                    << g_zone_vector[orig].m_ODAccessibilityMatrix.value_map[d]
            ...
            // dtalog.output() << "[DATA INFO] o: " << orig << ", total_attraction_utility =" <<
            // total_attraction_utility << '\n'; g_DTA_log_file << "[DATA INFO] o: " << orig << ",
            // total_attraction_utility =" << total_attraction_utility << '\n';
                ...
                for (int dest = 0; dest < g_zone_vector.size(); ++dest)  // d
                {
                    if (orig != dest)
                    {
                        if (g_zone_vector[dest].gravity_attraction > 0)
                        {
                            ...
                            // if (g_zone_vector[orig].m_ODAccessibilityMatrix.value_map[dest] <
                            // cut_off)
                            {
                                double ratio = g_zone_vector[orig]
                                                   .m_ODAccessibilityMatrix.disutility_map[dest] /
                                               total_attraction_utility;

                                g_zone_vector[orig].m_ODMatrix.value_map[dest] =
                                    g_zone_vector[orig].gravity_production * ratio;

                                if (trip_distribution_log_count < 100)
                                {
                                    // dtalog.output() << ", o: " << orig << ",d:" << dest << ",
                                    // ratio =" << ratio << g_DTA_log_file << ", o: " << orig <<
                                    // ",d:" << dest << ", ratio =" << ratio <<
                                ...
                                g_zone_vector[dest].gravity_est_attraction +=
                                    g_zone_vector[orig].m_ODMatrix.value_map[dest];
                            }
                        }
                    }
        ...
        for (int orig = 0; orig < g_zone_vector.size(); ++orig)  // o
        {
            for (int dest = 0; dest < g_zone_vector.size(); ++dest)  // d
            {
                if (g_zone_vector[orig].gravity_production >= 0)
                {
                    ...
                    {
                        float value = 1;
                        if (g_zone_vector[orig].m_ODMatrix.value_map.find(dest) !=
                            g_zone_vector[orig].m_ODMatrix.value_map.end())
                        {
...
void g_zone_to_access(Assignment& assignment)
{
    ...
    if (assignment.assignment_mode == 21 &&
        assignment.g_ModeTypeVector.size() > 0)  // zone2connector
    {
        int at = 0;
        // for each simple agent type
        if (assignment.g_ModeTypeVector[at].access_node_type.size() == 0)
        {
            // find the closest zone id

            if (debug_line_count <= 20)
            {
                dtalog.output() << "[DATA INFO] connector generation condition 1: agent type "
                                << assignment.g_ModeTypeVector[at].mode_type.c_str()
                                << " has access node type"
                                << assignment.g_ModeTypeVector[at].access_node_type.size() << '\n';
                g_DTA_log_file << "[DATA INFO] connector generation condition 1: agent type "
                               << assignment.g_ModeTypeVector[at].mode_type.c_str()
                               << " has access node type"
                               << assignment.g_ModeTypeVector[at].access_node_type.size() << '\n';
            ...
            {
                // first loop for mode_specific activity node with zone id
                if (g_node_vector[a_k].is_activity_node == 100)
                {
                    int zone_id = g_node_vector[a_k].zone_org_id;
                    int zone_seq_no = zone_seq_no =
                        assignment.g_zoneid_to_zone_seq_no_mapping[zone_id];
                    float access_distance = g_node_vector[a_k].access_distance;

                    if (debug_line_count <= 20)
                    {
                        dtalog.output() << "[DATA INFO] connector generation generation condition "
                                           "2: agent type no = "
                                        << at << " for node no. " << a_k
                                        << "as activity node with zone_id >=1\n";
                        g_DTA_log_file << "[DATA INFO] connector generation generation condition "
                                          "2: agent type no = "
                                       << at << " for node no. " << a_k
                                       << "as activity node with zone_id >=1\n";
                        // zone without multimodal access
                    ...
                    for (int i = 0; i < g_node_vector.size(); i++)
                    {
                        ...
                        {
                            // check allowed access code
                            if (assignment.g_ModeTypeVector[at].access_node_type.find(
                                    g_node_vector[i].node_type) != std::string::npos)
                            {
                                double zone_x = g_node_vector[a_k].x;
                                double zone_y = g_node_vector[a_k].y;

                                // test                                double near_by_distance_1 =
                                // g_calculate_p2p_distance_in_meter_from_latitude_longitude(-77.429293,
                                // 39.697895, -77.339847, 38.947676);

                                double distance =
                                    g_calculate_p2p_distance_in_meter_from_latitude_longitude(
                                        zone_x, zone_y, g_node_vector[i].x, g_node_vector[i].y);
                                // calculate the distance
                    ...
                    // preferred: access link within the range
                    if (access_node_seq_vector.size() > 0)
                    {
                        ...
                        if (access_node_distance_vector.size() > acecss_link_k)
                        {
                            ...
                            std::sort(access_node_distance_vector_temp.begin(),
                                      access_node_distance_vector_temp.end());

                            distance_k_cut_off_value = access_node_distance_vector_temp[std::max(
                                0, assignment.g_ModeTypeVector[at].acecss_link_k - 1)];
                            // distance_k can be dynamically determined based on the density of
                            // stops and stations at different areas, e.g.CBM vs. rual area
                        }

                        for (int an = 0; an < access_node_seq_vector.size(); an++)
                        {
                            if (access_node_distance_vector[an] <
                                distance_k_cut_off_value)  // within the shortest k ranage
                            {
                                if (g_node_vector[access_node_seq_vector[an]].is_boundary == 1)
                                    g_add_new_access_link(a_k, access_node_seq_vector[an],
                                                          access_node_distance_vector[an], at, -1);

                                if (g_node_vector[access_node_seq_vector[an]].is_boundary == -1)
                                    g_add_new_access_link(access_node_seq_vector[an], a_k,
                                                          access_node_distance_vector[an], at, -1);
