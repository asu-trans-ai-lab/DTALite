...
void g_program_stop()
{
    dtalog.output() << "Oops! DTALite encountered an issue and cannot proceed. We appreciate your "
                    << "understanding and would like to assist you in resolving any errors related "
                    << "to input files. Please double-check the input files mentioned in the logs "
                    << "for any potential errors or inconsistencies. If you need further "
                    << "assistance, please don't hesitate to reach out to us. We're here to help!"
                    << std::endl;
    g_DTA_log_file << "Oops! DTALite encountered an issue and cannot proceed. We appreciate your "
                   << "understanding and would like to assist you in resolving any errors related "
                   << "to input files. Please double-check the input files mentioned in the logs "
                   << "for any potential errors or inconsistencies. If you need further "
                   << "assistance, please don't hesitate to reach out to us. We're here to help!"
...
float g_timestamp_parser(string str)
{
    ...
    while (i < char_length)
    {
        ...
        // input to buf_ddhhmm until we meet the colon
        if (num_of_colons == 0 && ch != '_' && ch != ':')
...
bool CDTACSVParser::GetValueByFieldName(string field_name, string& value, bool required_field)
{
    if (FieldsIndices.find(field_name) == FieldsIndices.end())
    {
        if (required_field)
        {
            ...
            g_DTA_log_file << "[ERROR] Field " << field_name << " in file " << mFileName
                           << " does not exist. Please check the file.\n";
            g_program_stop();
        }
        return false;
...
bool g_read_a_line(FILE* f)
{
    while (1)
    {
        ...
        {
            /* terminate if it's end of line or end of file */
            if (ch == EOF)
                return false;
...
double g_calculate_p2p_distance_in_meter_from_latitude_longitude(double longitud1,
                                                                 double latitud1,
                                                                 double longitud2,
                                                                 double latitud2)
{
...
{
    //	g_RandomSeed = (g_LCG_a * g_RandomSeed + g_LCG_c) % g_LCG_M;  //m_RandomSeed is
    // automatically updated. 	return float(g_RandomSeed)/g_LCG_M;
...
bool g_get_line_intersection(double Ax,
                             double Ay,
                             double Bx,
                             double By,
                             double Cx,
                             ...
                             double Dy)
{
    ...
    if (Ax == Bx && Ay == By)
        return false;  // comment: C and D can be the same point from a vehile with the same
                       // timestamp
                                     ...
                                     std::vector<DTAGDPoint> subarea_shape_points)
{
    ...
    {
        double Cx = subarea_shape_points[i].x;
        double Cy = subarea_shape_points[i].y;
        double Dx = subarea_shape_points[i + 1].x;
        double Dy = subarea_shape_points[i + 1].y;
...
int g_test_point_in_polygon(DTAGDPoint Pt, std::vector<DTAGDPoint> V)
{
    ...
    for (int i = 0; i < n; i++)
    {
        // edge from V[i]  to V[i+1] an upward crossing
        if (((V[i].y <= Pt.y) && (V[i + 1].y > Pt.y)) || ((V[i].y > Pt.y) && (V[i + 1].y <= Pt.y)))
        {
            // a downward crossing
...
double g_Find_PPP_RelativeAngle(const DTAGDPoint* p1,
                                const DTAGDPoint* p2,
                                const DTAGDPoint* p3,
                                const DTAGDPoint* p4)
{
    ...
    int relative_angle = angle2 - angle1;
...
double g_GetPoint2LineDistance(const DTAGDPoint* pt,
                               const DTAGDPoint* FromPt,
                               const DTAGDPoint* ToPt)
{
    double LineLength = g_GetPoint2Point_Distance(FromPt, ToPt);
    double U =
        ((pt->x - ToPt->x) * (FromPt->x - ToPt->x) + (pt->y - ToPt->y) * (FromPt->y - ToPt->y)) /
    ...
    // intersection does not fall within the segment intersect
    if (U < 0.0 || U > 1.0)
        return min(distance_0, distance_2);
    else
        return distance_1;
}