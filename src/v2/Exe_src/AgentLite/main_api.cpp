...
public:
	...
	bool AllowAgentType(string agent_type)
	{
		...
		else
		{
			...
			{
				cout << "important: agent_type " << agent_type << " is prohibited " << " on link type " << link_type << endl;
				return false;
			}
...
//		g_debugging_flag = 1;
		...
		if (g_debug_level  >= 2)
			cout << "SP iteration k =  " << iteration_k << ": origin node: " << g_node_vector[origin_node].node_id << endl;
...
//			SEList_pop_front();  // remove current node FromID from the SE list
		...
		if (g_log_path >= 2)
		{ 
			cout << "SP:scan SE node: " << g_node_vector[from_node].node_id << " with " 
		...
		for (i = 0; i < NodeForwardStarArray[from_node].OutgoingLinkSize; i++)  // for each link (i,j) belong A(i)
			{
				...
				if (g_log_path >= 2)
				{
					cout << "SP:  checking outgoing node " << g_node_vector[to_node].node_id << endl;
				}
...
//Mark				new_distance = m_label_distance_array[from_node] + pLink->length;
				new_to_node_cost = m_node_label_cost[from_node] + m_link_genalized_cost_array[link_sqe_no];
							...
							if (g_log_path)
								{
									cout << "SP:  checking from node " << g_node_vector[from_node].node_id  << 
				...
				if (new_to_node_cost < m_node_label_cost[to_node]) // we only compare cost at the downstream node ToID at the new arrival time t
				{
					...
					if (g_log_path)
					{
						cout << "SP:  updating node: " << g_node_vector[to_node].node_id << " current cost:" << m_node_label_cost[to_node] <<
...
//					m_label_distance_array[to_node] = new_distance;
					...
					if (g_log_path)
						cout << "SP: add node " << g_node_vector[to_node].node_id << " new cost:" << new_to_node_cost <<
...
///// end of inline block
		...
		if (g_log_path)
			{ 
			cout <<  "SPtree at iteration k = " << iteration_k <<  " origin node: " << 
				...
				for (int i = 0; i < p_assignment->g_number_of_nodes; i++) //Initialization for all non-origin nodes
				{
					...
					{ 
					cout << "SP node: " << g_node_vector[i].node_id << " label cost " << m_node_label_cost[i] << "time "
		...
		while (parser.ReadRecord_Section())
		{
			...
			if (format_type.find("column") != string::npos)  // or muliti-column
			{
				...
				if (st != NULL)
				{
					...
					cout << "total_demand_volume is " << assignment.total_demand_volume << endl << endl;
				}
				else  //open file
				{
...
void g_ReadInputData(Assignment& assignment)
{
	...
	cout << "Step 1.2: Reading section [link_type] in setting.csv..." << endl;
	...
	if (parser_link_type.OpenCSVFile("settings.csv", false))
	{
		...
		while (parser_link_type.ReadRecord_Section())
		{
			if (parser_link_type.SectionName == "[link_type]")
			{
				...
				cout << "important: traffic_flow_code on link type " << element.link_type  << " is " << element.traffic_flow_code  << endl;
				...
				{
					cout << "important: agent type of " << element.agent_type_blocklist << " are prohibited " << " on link type " << element.link_type << endl;
				}
	...
	cout << "Step 1.3: Reading section [agent_type] in setting.csv..." << endl;
	...
	CCSVParser parser;
	cout << "Step 1.4: Reading node data in node.csv..."<< endl;
	...
	cout << "Step 1.5: Initializing O-D zone vector..." << endl;
	...
	if (assignment.assignment_mode == 5)  // gravity model.
	{
		...
		{
			cout << "File demand.csv cannot be opened." << endl;
			g_ProgramStop();
		}
		else
		{
			...
			for (int o = 0; o < g_zone_vector.size(); o++)  // o
			{
				if(g_zone_vector[o].obs_production >=0)
				{
					for (int d = 0; d < g_zone_vector.size(); d++)  // d
					{
						...
						cout << "o= " << g_zone_vector[o].zone_id << " d= " << g_zone_vector[d].zone_id << ":" <<  value << endl;
						}
	...
	cout << "Step 1.6: Reading link data in link.csv... " << endl;
		...
		for (int i = 0; i < g_node_vector.size(); i++)
		{
			...
			else
			{
				if (g_debug_level  == 3)
				{
					...
					cout << "node id= " << g_node_vector[i].node_id << " with " << g_node_vector[i].m_outgoing_link_seq_no_vector.size() << " outgoing links." << endl;
					for (int j = 0; j < g_node_vector[i].m_outgoing_link_seq_no_vector.size(); j++)
					{
	...
	if (parser_movement.OpenCSVFile("movement.csv", false))  // not required
	{
		while (parser_movement.ReadRecord())
		{
			...
			if (assignment.g_node_id_to_seq_no_map.find(node_id) == assignment.g_node_id_to_seq_no_map.end())
			{
				cout << "Error: node_id " << node_id << " in file movement.csv is not defined in node.csv." << endl;
			...
			{
				cout << "Error: ib_link_id " << ib_link_id.c_str() << " has not been defined in movement.csv. Please check link.csv." << endl;

			}

			if (assignment.g_link_id_map.find(ob_link_id) != assignment.g_link_id_map.end())
			{
				cout << "Error: ob_link_id " << ob_link_id.c_str() << " has not been defined in movement.csv. Please check link.csv." << endl;
		...
		cout << "Step XX: Reading movement.csv data with " << prohibited_count << " prohibited records." << endl;
...
void g_reload_service_arc_data(Assignment& assignment)
{
		...
		cout << "Step 1.7: Reading service arc in timing.csv..." << endl;
		if (parser_service_arc.OpenCSVFile("timing.csv", false))
		{
			while (parser_service_arc.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
			{
				...
				{
					cout << "Error: from_node_id in file timing.csv is not defined." << endl;
					continue;
				}
				...
				if (assignment.g_node_id_to_seq_no_map.find(from_node_id) == assignment.g_node_id_to_seq_no_map.end())
				{
					cout << "Error: from_node_id " << from_node_id << " in file timing.csv is not defined in node.csv." << endl;

					continue; //has not been defined
				}
				if (assignment.g_node_id_to_seq_no_map.find(to_node_id) == assignment.g_node_id_to_seq_no_map.end())
				{
					cout << "Error: to_node_id " << to_node_id << " in file timing.csv is not defined in node.csv." << endl;
					continue; //has not been defined
				}
				...
				{
					cout << "Error: Link " << from_node_id << "->" << to_node_id << " in file timing.csv is not defined in link.csv." << endl;
					continue;
				}
				...
				{
					cout << "Error: Field time_window in file timing.csv cannot be read." << endl;
					g_ProgramStop();
					break;
				}
				...
				if (assignment.g_number_of_timing_arcs % 10000 == 0)
					cout << "reading " << assignment.g_number_of_timing_arcs << " timing_arcs.. " << endl;
			}
			...
			while (parser.ReadRecord_Section())
			{
				...
				if (parser.SectionName == "[capacity_scenario]")
				{
					...
					assignment.g_number_of_timing_arcs++;
						...
						cout << "reading " << assignment.g_number_of_timing_arcs << " capacity reduction scenario.. " << endl;
	...
	// we now know the number of links
		cout << "number of timing records = " << assignment.g_number_of_timing_arcs << endl << endl;
	...
	for (int o = 0; o < g_zone_vector.size(); o++)  // o
	{
		...
		for (l = 0; l < number_of_links; l++)
		{
			...
			if (g_link_vector[l].obs_count >= 1)  // with data
			{
				...
				if (g_debug_level  == 2)
				{ 
				cout << "link " << g_node_vector [g_link_vector[l].from_node_seq_no].node_id <<
					"->" << g_node_vector[g_link_vector[l].to_node_seq_no].node_id
		...
		for (int o = 0; o < g_zone_vector.size(); o++)  // o
		{
			if (g_zone_vector[o].obs_attraction >= 1)  // with observation
			{
				...
				if (g_debug_level  == 2)
				{
					cout << "zone " << g_zone_vector[o].zone_id << "A: obs:" << g_zone_vector[o].obs_attraction <<
			...
			if (g_zone_vector[o].obs_production >= 1)  // with observation
			{
				...
				if (g_debug_level  == 2)
				{
				cout << "zone " << g_zone_vector[o].zone_id << "P: obs:" << g_zone_vector[o].obs_production <<
		...
		cout << "ODME #" << iteration_no<< " total abs gap= " << total_gap <<
...
void g_column_pool_optimization(Assignment& assignment, int column_updating_iterations)
{
	...
	{ 
		cout << "Current iteration number: " << n << endl;
		...
		{
			cout << "link: " << g_node_vector[g_link_vector[i].from_node_seq_no].node_id << "-->"
...
void g_output_simulation_result(Assignment& assignment)
{
	...
	else
	{
		...
		cout << "writing data for " << zone_size << "  zones " << endl;

		for (o = 0; o < zone_size; o++)
		{ 
			if(g_zone_vector[o].zone_id %100 ==0)
			cout << "o zone id =  " << g_zone_vector[o].zone_id << endl;
...
void g_assign_computing_tasks_to_memory_blocks(Assignment& assignment)
{
	...
	cout << "Step 2: Assigning computing tasks to memory blocks..." << endl;
...
double network_assignment(int iteration_number, int assignment_mode, int column_updating_iterations)
{
	...
	cout << endl;
	cout << "Step 3: Column Generation for Traffic Assignment..." << endl;
	cout << "Total Column Generation iteration: " << assignment.g_number_of_column_generation_iterations << endl;
	for (int iteration_number = 0; iteration_number < assignment.g_number_of_column_generation_iterations; iteration_number++)
	{
		cout << endl;
		cout << "Current iteration number:" << iteration_number << endl;
		end_t = clock();
		iteration_t = end_t - start_t;
		cout << "Current CPU time: " << iteration_t / 1000.0 << " s" << endl;

// commment out for DLL version
		//if (signal_updating_iterations >=1 && iteration_number >= signal_updating_iterations)
		//{
		//cout << "use SignalAPI to recalibrate signal timing at iteration " << iteration_number << endl;
...
//		cout << "column generation with CPU time " << cumulative_cp / 1000.0 << " s; " << endl;
	...
	cout << "Step 4: Column Pool Updating" << endl;
	cout << "Total Column Pool Updating iteration: " << column_updating_iterations << endl;
	start_t = clock();
	...
	if (assignment.assignment_mode == 2)
	{
		cout << "Step 5: Simulation for traffic assignment.." << endl;
	...
	{
		cout << "Step 6: O-D estimation for traffic assignment.." << endl;
...
void Assignment::AllocateLinkMemory4Simulation()
{
	...
	cout << "allocate 2D dynamic memory LinkOutFlowCapacity..." << endl;
...
#pragma omp parallel for
	...
	cout << "End of initializing time dependent capacity data." << endl;

}

void Assignment::DeallocateLinkMemory4Simulation()
{
//	cout << "deallocate 2D dynamic memory m_LinkOutFlowCapacity..." << endl;
...
//	cout << "deallocate 2D dynamic memory m_LinkCumulativeArrival..." << endl;
...
//	cout << "deallocate 2D dynamic memory m_LinkCumulativeDeparture..." << endl;
...
//	cout << "deallocate 2D dynamic memory m_LinkTDTravelTime..." << endl;
...
//	cout << "deallocate 2D dynamic memory m_LinkTDWaitingTime..." << endl;
...
void Assignment::STTrafficSimulation()
{
	...
	cout << "number of simulation zones:" << zone_size << endl;
...
//									est_production_dev = est_production - obs_production;
								...
								if (g_log_odme == 1)
								{ 
								cout << "OD " << o << "-> " << d << " path id:" << i << ", prev_vol"
