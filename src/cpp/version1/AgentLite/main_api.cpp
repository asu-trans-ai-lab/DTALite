...
public:
	...
	bool OpenCSVFile(string fileName, bool b_required)
	{
		...
		else
		{
			if (b_required)
			{
				...
				log_out << "File " << fileName << " does not exist. Please check." << endl;
				//g_ProgramStop();
			}
			return false;
		}
	...
	template <class T> bool GetValueByFieldName(string field_name, T& value, bool required_field = true, bool NonnegativeFlag = true)
	{
		...
		if (FieldsIndices.find(field_name) == FieldsIndices.end())
		{
			if (required_field)
			{
				log_out << "Field " << field_name << " in file " << mFileName << " does not exist. Please check the file." << endl;
...
T **AllocateDynamicArray(int nRows, int nCols)
{
	...
	{
		log_out << "Error: insufficient memory.";
		g_ProgramStop();
...
T ***Allocate3DDynamicArray(int nX, int nY, int nZ)
{
	...
	for (int x = 0; x < nX; x++)
	{
		if (x % 1000 == 0)
		{
			log_out << "allocating 3D memory for " << x << endl;
		}
...
T ****Allocate4DDynamicArray(int nM, int nX, int nY, int nZ)
{
	...
	{
		if (m % 1000 == 0)
			log_out << "allocating 4D memory for " << m << " zones" << endl;
...
public:
	...
	bool AllowAgentType(string agent_type)
	{
		...
		else
		{
			...
			{
				log_out << "important: agent_type " << agent_type << " is prohibited " << " on link type " << link_type << endl;
				return false;
			}
		...
		for (int i = 0; i < p_assignment->g_number_of_nodes; i++) //Initialization for all non-origin nodes
		{
			...
			for (int j = 0; j < g_node_vector[i].m_outgoing_link_seq_no_vector.size(); j++)
			{
				...
				{ 
					...
					{
						log_out << " Error: outgoing_link_size >= _MAX_LINK_SIZE_FOR_A_NODE" << endl;
						g_ProgramStop();
					}
			...
			for (int i = 0; i < g_node_vector.size(); i++)
			{
				...
				{ // we need to make sure we only create two way connectors between nodes and zones 
					...
					for (int j = 0; j < NodeForwardStarArray[i].OutgoingLinkSize; j++)
					{
						...
						log_out << "  outgoing node = " << g_node_vector[g_link_vector[link_seq_no].to_node_seq_no].node_id << endl;
					}
				}
				else
				{
...
//		g_debugging_flag = 1;
		...
		{
			log_out << "label correcting for zone " << origin_zone <<  " in processor " << processor_id <<  endl;
		}
		...
		for (i = 0; i < NodeForwardStarArray[from_node].OutgoingLinkSize; i++)  // for each link (i,j) belong A(i)
			{
				...
				if (g_node_vector[from_node].prohibited_movement_size >= 1)
				{
					if (pred_link_seq_no >= 0)
					{
						...
						{
							log_out << "prohibited movement " << movement_string << " will not be used " << endl;
							continue;
						}
		...
		if (log_out.log_path)
			...
			log_out <<  "SPtree at iteration k = " << iteration_k <<  " origin node: " << 
				...
				for (int i = 0; i < p_assignment->g_number_of_nodes; i++) //Initialization for all non-origin nodes
				{
					...
					{ 
					log_out << "SP node: " << g_node_vector[i].node_id << " label cost " << m_node_label_cost[i] << "time "
...
//{
//	log_out << "The number of global minutes is: " << g_global_minute[i] << " minutes" << endl;
//}
...
void g_ProgramStop()
{
	...
	log_out << "STALite Program stops. Press any key to terminate. Thanks!" << endl;
	getchar();
	exit(0);
};
...
//	fprintf(g_pFileOutputLog, "number of zones =,%lu\n", g_zone_vector.size());
	...
	CCSVParser parser;
	log_out << endl;
	log_out << "Step 1.8: Reading file section [demand_file_list] in setting.csv..." << endl;
...
void g_ReadInputData(Assignment& assignment)
{
	...
	log_out << "Step 1: Reading input data" << endl;
	log_out << "Step 1.1: Reading section [demand_period] in setting.csv..." << endl;
	...
	if (parser_demand_period.OpenCSVFile("settings.csv", false))
	{
		...
		while (parser_demand_period.ReadRecord_Section())
		{
			...
			{
				log_out << "Error: Field demand_period in file demand_period cannot be read." << endl;
				g_ProgramStop();
				break;
			}
			...
			{ 
				log_out << "Error: Field time_period in file demand_period cannot be read." << endl;
				g_ProgramStop();
				break;
			}
			...
			if (global_minute_vector.size() == 2)
			{
				...
				//log_out << global_minute_vector[0] << endl;
				//log_out << global_minute_vector[1] << endl;
			}
		...
		{
		log_out << "Error:  Section demand_period has no information." << endl;
		g_ProgramStop();
		}
	...
	{
			log_out << "Error: File settings.csv cannot be opened.\n It might be currently used and locked by EXCEL." << endl;
			g_ProgramStop();

	}

	log_out << "number of demand periods = " << assignment.g_DemandPeriodVector.size() << endl;

	assignment.g_number_of_demand_periods = assignment.g_DemandPeriodVector.size();
	//step 1:read demand type file

	log_out << "Step 1.2: Reading section [link_type] in setting.csv..." << endl;
		...
		while (parser_link_type.ReadRecord_Section())
		{
			if (parser_link_type.SectionName == "[link_type]")
			{
				...
				if (parser_link_type.GetValueByFieldName("link_type", element.link_type) == false)
				{
					if (line_no == 0)
					{
						log_out << "Error: Field link_type cannot be found in file link_type.csv." << endl;
						g_ProgramStop();
					}
					else
					{  // read empty line
				...
				{
					log_out << "Error: Field link_type " << element.link_type << " has been defined more than once in file link_type.csv." << endl;
					g_ProgramStop();
				...
				log_out << "important: traffic_flow_code on link type " << element.link_type  << " is " << element.traffic_flow_code  << endl;
				...
				{
					log_out << "important: agent type of " << element.agent_type_blocklist << " are prohibited " << " on link type " << element.link_type << endl;
				}
	...
	log_out << "number of link types = " << assignment.g_LinkTypeMap.size() << endl;

	CCSVParser parser_agent_type;
	log_out << "Step 1.3: Reading section [agent_type] in setting.csv..." << endl;
	...
	if (parser_agent_type.OpenCSVFile("settings.csv", false))
	{
		...
		{
			log_out << "Error: Section agent_type does not contain information." << endl;
		}
	...
	{
		log_out << "Error: agent_type = " << assignment.g_AgentTypeVector.size() << " in section agent_type is too large. " << "_MAX_AGNETTYPES = " << _MAX_AGNETTYPES << "Please contact program developers!";

		g_ProgramStop();
	}


	log_out << "number of agent typess = " << assignment.g_AgentTypeVector.size() << endl;
	...
	if (parser.OpenCSVFile("node.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			if (assignment.g_number_of_nodes % 5000 == 0)
				log_out << "reading " << assignment.g_number_of_nodes << " nodes.. " << endl;
		}

		log_out << "number of nodes = " << assignment.g_number_of_nodes << endl;
	...
	log_out << "Step 1.6: Reading link data in link.csv... " << endl;
	if (parser_link.OpenCSVFile("link.csv", true))
	{
		while (parser_link.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			{
				log_out << "link type " << link.link_type << " in link.csv is not defined for link " << from_node_id << "->"<< to_node_id << " in link_type.csv" <<endl;
				g_ProgramStop();
...
//				parser_link.GetValueByFieldName(VDF_field_name, link.VDF_period[tau].theta,false, false);  // theta was used only for the illustration case in Breass paradox
			...
			if (assignment.g_number_of_links % 10000 == 0)
				log_out << "reading " << assignment.g_number_of_links << " links.. " << endl;
		}
	...
	if (parser_movement.OpenCSVFile("movement.csv", false))  // not required
	{
		while (parser_movement.ReadRecord())
		{
			...
			if (assignment.g_node_id_to_seq_no_map.find(node_id) == assignment.g_node_id_to_seq_no_map.end())
			{
				log_out << "Error: node_id " << node_id << " in file movement.csv is not defined in node.csv." << endl;
			...
			{
				log_out << "Error: ib_link_id " << ib_link_id.c_str() << " has not been defined in movement.csv. Please check link.csv." << endl;

			}

			if (assignment.g_link_id_map.find(ob_link_id) != assignment.g_link_id_map.end())
			{
				log_out << "Error: ob_link_id " << ob_link_id.c_str() << " has not been defined in movement.csv. Please check link.csv." << endl;
...
void g_reload_service_arc_data(Assignment& assignment)
{
		...
		log_out << "Step 1.7: Reading service arc in timing.csv..." << endl;
		if (parser_service_arc.OpenCSVFile("timing.csv", false))
		{
			while (parser_service_arc.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
			{
				...
				if (assignment.g_node_id_to_seq_no_map.find(from_node_id) == assignment.g_node_id_to_seq_no_map.end())
				{
					log_out << "Error: from_node_id " << from_node_id << " in file timing.csv is not defined in node.csv." << endl;

					continue; //has not been defined
				}
				if (assignment.g_node_id_to_seq_no_map.find(to_node_id) == assignment.g_node_id_to_seq_no_map.end())
				{
					log_out << "Error: to_node_id " << to_node_id << " in file timing.csv is not defined in node.csv." << endl;
					continue; //has not been defined
				}
				...
				{
					log_out << "Error: Link " << from_node_id << "->" << to_node_id << " in file timing.csv is not defined in link.csv." << endl;
					continue;
				}
				...
				{
					log_out << "Error: Field time_window in file timing.csv cannot be read." << endl;
					g_ProgramStop();
					break;
				}
				...
				if (assignment.g_number_of_timing_arcs % 10000 == 0)
					log_out << "reading " << assignment.g_number_of_timing_arcs << " timing_arcs.. " << endl;
			}
	...
	// we now know the number of links
		log_out << "number of timing records = " << assignment.g_number_of_timing_arcs << endl << endl;
...
void g_column_pool_optimization(Assignment& assignment, int column_updating_iterations)
{
	...
	{ 
		...
		{
			log_out << "link: " << g_node_vector[g_link_vector[i].from_node_seq_no].node_id << "-->"
...
void g_output_simulation_result(Assignment& assignment)
{
	...
	{
		log_out << "File link_performance.csv cannot be opened." << endl;
		g_ProgramStop();
	}
	else
	{
		...
		for (int l = 0; l < g_link_vector.size(); l++) //Initialization for all nodes
		{
			...
			for (int tau = 0; tau < assignment.g_number_of_demand_periods; tau++)
			{
				if (g_link_vector[l].flow_volume_per_period[tau] <= 0.0001)  // to be tested
					continue;
...
//							waiting_time_in_min = queue / (max(1, volume));
	...
	{
	log_out << "writing agent.csv.." << endl;
	...
	{
		log_out << "File agent.csv cannot be opened." << endl;
		g_ProgramStop();
	}
	else
	{
		...
		log_out << "writing data for " << zone_size << "  zones " << endl;
					...
					for (tau = 0; tau < demand_period_size; tau++)
					{
						...
						if (p_column->od_volume > 0)
						{
							...
							for (it = it_begin;it != it_end; it++)
							{
								...
								log_out << "writing " << count/1000 << "K agents with CPU time " << iteration_t / 1000.0 << " s" << endl;
								}
...
void g_output_simulation_result_for_signal_api(Assignment& assignment)
{
	...
	if (g_pFileLinkMOE == NULL)
	{
		log_out << "File link_performance_sig.csv cannot be opened." << endl;
		g_ProgramStop();
	}
	else
	{
...
void g_assign_computing_tasks_to_memory_blocks(Assignment& assignment)
{
	...
	log_out << "Step 2: Assigning computing tasks to memory blocks..." << endl;
	...
	log_out << "There are " << g_NetworkForSP_vector.size() << " networks in memory." << endl;
}
...
void NetworkForSP::backtrace_shortest_path_tree(Assignment& assignment, int iteration_number_outterloop, int o_node_index)
{
		...
		//{
		//	log_out << "backtracing for zone " << m_origin_zone_seq_no << endl;
		//}
			...
			if (g_node_vector[i].zone_id >= 1)
			{
				...
				if (ODvolume > 0.000001)
				{
					...
					while (current_node_seq_no >= 0 && current_node_seq_no < number_of_nodes)
					{
						...
						{
							log_out << "Error: l_node_size >= temp_path_node_vector_size" << endl;
							g_ProgramStop();
						}
...
double network_assignment(int iteration_number, int assignment_mode, int column_updating_iterations, int signal_updating_iterations)
{
	...
	log_out << endl;
	log_out << "Step 3: Column Generation for Traffic Assignment..." << endl;
	log_out << "Total Column Generation iteration: " << assignment.g_number_of_column_generation_iterations << endl;
	for (int iteration_number = 0; iteration_number < assignment.g_number_of_column_generation_iterations; iteration_number++)
	{
		log_out << endl;
		log_out << "Current iteration number:" << iteration_number << endl;
		end_t = clock();
		iteration_t = end_t - start_t;
		log_out << "Current CPU time: " << iteration_t / 1000.0 << " s" << endl;

		if (signal_updating_iterations >=1 && iteration_number >= signal_updating_iterations)
		{
		log_out << "use SignalAPI to recalibrate signal timing at iteration " << iteration_number << endl;
...
//		log_out << "Link update with CPU time " << iteration_t / 1000.0 << " s; " << (end_t - start_t) / 1000.0 << " s" << endl;
...
//		log_out << "LC with CPU time " << cumulative_lc / 1000.0 << " s; " << endl;
//		log_out << "column generation with CPU time " << cumulative_cp / 1000.0 << " s; " << endl;
	...
	log_out << "Step 4: Column Pool Updating" << endl;
	log_out << "Total Column Pool Updating iteration: " << column_updating_iterations << endl;
	start_t = clock();
	...
	if (assignment.assignment_mode == 2)
	{
		log_out << "Step 5: Simulation for traffic assignment.." << endl;
	...
	{
		log_out << "Step 6: O-D estimation for traffic assignment.." << endl;
	...
	log_out << "CPU Running Time for column pool updating: " << total_t / 1000.0 << " s" << endl;
	...
	end_t = clock();
	total_t = (end_t - start_t);
	log_out << "Output for assignment with " << assignment.g_number_of_column_generation_iterations << " iterations. Traffic assignment completes!" << endl;
	log_out << "CPU Running Time for outputting simulation results: " << total_t / 1000.0 << " s" << endl;

	log_out << "free memory.." << endl;
	g_node_vector.clear();
...
void Assignment::AllocateLinkMemory4Simulation()
{
	...
	log_out << "allocate 2D dynamic memory LinkOutFlowCapacity..." << endl;
...
#pragma omp parallel for
	...
	log_out << "End of initializing time dependent capacity data." << endl;

}

void Assignment::DeallocateLinkMemory4Simulation()
{
//	log_out << "deallocate 2D dynamic memory m_LinkOutFlowCapacity..." << endl;
...
//	log_out << "deallocate 2D dynamic memory m_LinkCumulativeArrival..." << endl;
...
//	log_out << "deallocate 2D dynamic memory m_LinkCumulativeDeparture..." << endl;
...
//	log_out << "deallocate 2D dynamic memory m_LinkTDTravelTime..." << endl;
...
//	log_out << "deallocate 2D dynamic memory m_LinkTDWaitingTime..." << endl;
...
void Assignment::STTrafficSimulation()
{
	...
	for (o = 0; o < zone_size; o++)
	{
		if (o % 100 == 0)
		{
			log_out << "generating " << g_agent_simu_vector.size()/1000 << " K agents for "  << o << "  zones " << endl;
		}
	...
	log_out << "number of simulation zones:" << zone_size << endl;
	...
	for (int t = 0; t < g_number_of_simulation_intervals; t++)  // first loop for time t
	{
		...
		if (t % (number_of_simu_interval_per_min*10) == 0)
			log_out << "simu time= " << t / number_of_simu_interval_per_min << " min, CA = " << TotalCumulative_Arrival_Count << " CD=" << TotalCumulative_Departure_Count << endl;
...
//										log_out << "spatical queue in the next link is blocked on link seq  " <<  g_node_vector[pNextLink->from_node_seq_no].node_id  << " -> " << g_node_vector[pNextLink->to_node_seq_no].node_id  <<endl;
										break;
										}
									}
	...
	if (parser_measurement.OpenCSVFile("measurement.csv", true))
	{
		while (parser_measurement.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			if (measurement_type == "link")
			{
				...
				{
					log_out << "Error: from_node_id " << from_node_id << " in file measurement.csv is not defined in node.csv." << endl;

					continue; //has not been defined
				}
				if (g_node_id_to_seq_no_map.find(to_node_id) == assignment.g_node_id_to_seq_no_map.end())
				{
					log_out << "Error: to_node_id " << to_node_id << " in file measurement.csv is not defined in node.csv." << endl;
					continue; //has not been defined
				}
